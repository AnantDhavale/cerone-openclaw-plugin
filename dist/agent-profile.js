...
function inferRequiredCapability(toolName) {
...
function minimalCapabilities(requiredCapability) {
    return [requiredCapability];
}
function describeWorkspaceTarget(event) {
    const firstPath = Array.isArray(event.derivedPaths)
        ? event.derivedPaths.find((entry) => typeof entry === "string" && entry.trim().length > 0)
        : undefined;
    if (!firstPath) {
...
function inferPurpose(requiredCapability, event, workspaceTarget) {
    switch (requiredCapability) {
        case "file_read":
            return (`Perform ${event.toolName} operations to read files from a codebase and inspect ${workspaceTarget} ` +
                "for source code analysis, configuration review, debugging, and implementation planning.");
        case "file_write":
            return (`Perform ${event.toolName} operations to update project files within ${workspaceTarget} ` +
                "for software engineering changes, fixes, and implementation tasks.");
        case "api_call":
            return (`Perform ${event.toolName} operations to call development and service APIs needed for ` +
                "software engineering workflows, diagnostics, and implementation tasks.");
        case "network_access":
            return (`Perform ${event.toolName} operations to access network resources relevant to ${workspaceTarget} ` +
                "for software engineering research, dependency inspection, and debugging.");
        case "db_read":
            return (`Perform ${event.toolName} operations to read database records needed for debugging, ` +
                "system analysis, and software engineering investigation.");
        case "db_write":
            return (`Perform ${event.toolName} operations to update database records required for controlled ` +
                "software engineering workflows and operational fixes.");
        default:
            return (`Perform ${event.toolName} operations within OpenClaw to work with ${workspaceTarget} ` +
                "for software engineering, debugging, and workflow tasks.");
    }
}
export function resolveAgentProfile(config, event) {
    const requiredCapability = inferRequiredCapability(event.toolName);
    const capabilities = config.agentCapabilities.length > 0
        ? config.agentCapabilities
        : minimalCapabilities(requiredCapability);
    const workspaceTarget = describeWorkspaceTarget(event);
    const purpose = config.agentPurpose ??
